 ...
 * Added: detectionActive flag — when false, /command always returns IDLE
 */
...
function decideCommand(db, scored) {
  ...
  if (pump.isOn) return { command: "IDLE", reason: "pump_already_on", score: scored.score };

  // ── Detection gate ──
  if (!db.detectionActive) return { command: "IDLE", reason: "detection_off", score: scored.score };

  if (manual.pending) {
    return { command: "WATER", durationMs: settings.pumpDurationMs || 7000, reason: "manual", requestId: manual.id, score: scored.score };
  }

  if (pump.lastWateredAt) {
    const elapsed = now - new Date(pump.lastWateredAt).getTime();
    if (elapsed < (settings.cooldownMs || 3600000)) return { command: "IDLE", reason: "cooldown", score: scored.score };
  }

  if (!scored.thresholdMet) return { command: "IDLE", reason: "score_below_threshold", score: scored.score };

  const pumpFlow = settings.pumpFlowMlPerSec || 20;
  const waterMl  = Math.max(settings.minWaterMl || 15, Math.min(settings.maxWaterMl || 120, scored.recommendedWaterMl));
  return { command: "WATER", durationMs: Math.round((waterMl / pumpFlow) * 1000), volumeMl: waterMl, reason: "auto_score", score: scored.score };
}
...
app.post("/config", async (req, res) => {
  const { plantType, cooldownMs, pumpDurationMs, potSizeCategory, plantSizeCategory, pumpFlowMlPerSec, minWaterMl, maxWaterMl } = req.body;
  if (plantType && !isSupportedPlant(plantType)) return res.status(400).json({ error: "Unsupported plant type", supportedPlants: getPlantNames() });
...
app.post("/detection", async (req, res) => {
  ...
  if (typeof active !== "boolean") return res.status(400).json({ error: "body must be { active: true | false }" });
...
app.post("/sensor-data", async (req, res) => {
  ...
  const db = await updateDb((cur) => {
    const pumpStateChanged = typeof payload.pumpState === "boolean" && payload.pumpState !== cur.pumpStatus.isOn;
    const next = {
      ...cur,
      sensorData: keepRecentItems([...cur.sensorData, entry], 500),
      pumpStatus: { ...cur.pumpStatus, isOn: typeof payload.pumpState === "boolean" ? payload.pumpState : cur.pumpStatus.isOn, lastChangedAt: pumpStateChanged ? timestamp : cur.pumpStatus.lastChangedAt },
...
app.get("/command", async (_req, res) => {
  const db = await readDb();
  if (!db.sensorData || db.sensorData.length === 0) return res.json({ command: "IDLE", reason: "no_sensor_data", score: 0 });
  if (!db.detectionActive) return res.json({ command: "IDLE", reason: "detection_off", score: 0, detectionActive: false });
  ...
  res.json({ command: decision.command, durationMs: decision.durationMs || 0, volumeMl: decision.volumeMl || 0, reason: decision.reason, requestId: decision.requestId || null, score: decision.score, threshold: getPlantProfile(db.currentPlant).scoreThreshold, detectionActive: true });
});
...
app.post("/watering-log", async (req, res) => {
  ...
  const logEntry = { deviceId: payload.deviceId || "esp32-plant-01", event, reason: payload.reason || "automatic", durationMs: payload.durationMs ?? 0, volumeMl: payload.volumeMl ?? null, score: payload.score ?? null, requestId: payload.requestId || null, timestamp, serverReceivedAt: new Date().toISOString() };

  const db = await updateDb((cur) => {
    const next = { ...cur, wateringLogs: keepRecentItems([...cur.wateringLogs, logEntry], 300), pumpStatus: { ...cur.pumpStatus } };
    ...
    if (logEntry.requestId && cur.manualWaterRequest.pending && cur.manualWaterRequest.id === logEntry.requestId) {
...
app.post("/manual-watering", async (_req, res) => {
  ...
  const db = await updateDb((cur) => ({ ...cur, manualWaterRequest: { pending: true, id: requestId, requestedAt: new Date().toISOString(), consumedAt: null } }));
